use super::{
    ...
    raw_jagged_slice::RawJaggedSlice, raw_jagged_storage::RawJaggedStorage, raw_slice::RawSlice,
    raw_vec::RawVec,
};
    ...
    X: JaggedIndexer,
{
    fn new(arrays: S, indexer: X, droppable: bool, total_len: Option<usize>) -> Self {
        let len = total_len.unwrap_or_else(|| arrays.arr_lengths().sum());
    ...
    pub fn jagged_index(&self, flat_index: usize) -> Option<JaggedIndex> {
        match flat_index.cmp(&self.len) {
            ...
            Ordering::Equal => match self.num_arrays() == 0 {
                true => None,
                false => {
                    let f = self.num_arrays() - 1;
                    let i = self.arrays.arr_length_of(f);
    ...
    fn first_and_last_ptrs_of(&self, f: usize) -> [*const S::Item; 2] {
        self.arrays.first_and_last_ptrs_of(f)
    }
}
