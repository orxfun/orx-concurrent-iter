use super::chunk_puller_owned::ChunkPullerJaggedOwned;
use crate::{
    ConcurrentIter, ExactSizeConcurrentIter,
    implementations::jagged::{
        iter::{RawJaggedIterOwned, RawJaggedSliceIterOwned},
        jagged_indexer::JaggedIndexer,
        raw_jagged::RawJagged,
    },
};
    ...
    X: JaggedIndexer + Send + Sync,
{
    ...
    fn into_seq_iter(mut self) -> Self::SequentialIter {
        ...
        let mut jagged_to_drop = self.jagged.clone(); // note that this does not clone data, just the metadata
        // SAFETY: `num_taken` elements are already returned by the concurrent iterator;
        // hence, should not be removed again.
        unsafe { jagged_to_drop.set_num_taken(Some(num_taken)) };

        // SAFETY: remaining elements will be dropped by `jagged_to_drop`; hence,
        // self.jagged must not drop any elements.
        unsafe { self.jagged.set_num_taken(None) };
