...
fn enumeration() {
    ...
    let iter = vec.concurrent_iter().copied();
...
fn copied_empty_slice<K: Enumeration>(_: K, nt: usize) {
    let vec = Vec::<usize>::new();
    let slice = vec.as_slice();
    let iter = slice
        .into_concurrent_iter()
        .into_enumeration_of::<K>()
        .copied();
...
fn next<K: Enumeration>(_: K, n: usize, nt: usize)
where
    for<'a> <K::Element as Element>::ElemOf<usize>: PartialEq + Ord + Debug,
{
    let vec: Vec<_> = (0..n).map(|x| x + 10).collect();
    let iter = vec.concurrent_iter().into_enumeration_of::<K>().copied();
    ...
    let mut expected: Vec<_> = (0..n).map(|i| K::new_element(i, vec[i])).collect();
    expected.sort();
    ...
    for<'a> <K::Element as Element>::ElemOf<usize>: PartialEq + Ord + Debug,
{
    ...
    for i in 0..n {
        let c = (i / 7) * 7;
        expected.push(K::new_element(c, vec[i]));
    }
    expected.sort();
    let mut collected = bag.into_inner().to_vec();
    collected.sort();
...
fn into_seq_iter<K: Enumeration>(_: K, n: usize, nt: usize, until: usize) {
    let vec: Vec<_> = (0..n).map(|x| x + 10).collect();
    let iter = vec.concurrent_iter().into_enumeration_of::<K>().cloned();
