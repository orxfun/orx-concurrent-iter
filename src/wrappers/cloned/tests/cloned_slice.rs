...
fn enumeration() {
    ...
    let iter = slice.into_concurrent_iter().cloned();
...
fn cloned_empty_slice<K: Enumeration>(_: K, nt: usize) {
    ...
    for<'a> <K::Element as Element>::ElemOf<String>: PartialEq + Ord + Debug,
{
    ...
    let slice = vec.as_slice();
    let iter = slice.concurrent_iter().into_enumeration_of::<K>().cloned();
    ...
    let mut expected: Vec<_> = (0..n)
        .map(|i| K::new_element(i, slice[i].clone()))
        .collect();
    expected.sort();
    ...
    for<'a> <K::Element as Element>::ElemOf<String>: PartialEq + Ord + Debug,
{
    ...
    for i in 0..n {
        let c = (i / 7) * 7;
        expected.push(K::new_element(c, slice[i].clone()));
    }
    expected.sort();
    let mut collected = bag.into_inner().to_vec();
    collected.sort();
...
fn skip_to_end<K: Enumeration>(_: K, n: usize, nt: usize) {
    ...
    let mut expected: Vec<_> = (0..until).map(|i| slice[i].clone()).collect();
    expected.sort();
    let mut collected = bag.into_inner().to_vec();
    collected.sort();
